...
public partial class OverrideSystem:GameSystemBase,IPreDeserialize {
 ...
 private int _catalogRecoveryPasses,_catalogRecoveryNext;
 ...
 private void Apply(bool catalogOnly=false){using(RuntimeDiagnostics.Measure("Catalog.Scan"))ScanCatalog();} private void ScanCatalog(){
  if(Mod.Settings==null||!_allow)return;Catalog.Unavailable=0;
  using(RuntimeDiagnostics.Measure("Catalog.ReadWait"))EntityManager.CompleteDependencyBeforeRO<SubMesh>();
