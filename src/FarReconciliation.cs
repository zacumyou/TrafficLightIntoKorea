...
public partial class FarSignalSystem {
 ...
 private void TickRetries(){float now=UnityEngine.Time.realtimeSinceStartup;var due=new List<Entity>();foreach(var pair in _retryAt)if(pair.Value<=now)due.Add(pair.Key);foreach(var owner in due){_retryAt.Remove(owner);Queue(owner);}}
