...
public partial class TrafficSignSystem:GameSystemBase {
 ...
 private int _processed,_created;private double _lastMs,_maxMs;internal static string Status="";
 ...
 private void Hidden(Entity e,bool hide){if(!Live(e))return;bool old=EntityManager.HasComponent<Hidden>(e);if(old==hide)return;if(hide)EntityManager.AddComponent<Hidden>(e);else EntityManager.RemoveComponent<Hidden>(e);Mark(e);}
 ...
 protected override void OnUpdate(){using(RuntimeDiagnostics.Measure("TrafficSignSystem.OnUpdate")){if(_saving)return;if(!_ready||Mod.Settings==null||!Mod.Settings.Enabled){if(_round.Count>0)Clear();return;}
  ...
  for(int i=0;i<4&&_round.Count>0&&timer.Elapsed.TotalMilliseconds<2;i++){if(_cursor>=_round.Count)_cursor=0;var e=_round[_cursor++];Reconcile(e.Parent);_processed++;}
  ...
  _lastMs=timer.Elapsed.TotalMilliseconds;_maxMs=Math.Max(_maxMs,_lastMs);Status="controllers="+SignPhaseController.Available+" assets="+_assets.Count+"/14 approaches="+_roadStates.Count+" parents="+_round.Count+" pending="+_pending.Count+" processed="+_processed+" created="+_created+" ms="+_lastMs.ToString("F2")+" peak="+_maxMs.ToString("F2");
 }}
 private void Reconcile(Entity parent){
  ...
  for(int i=0;i<entry.Mounts.Length;i++){
   ...
   if(EntityManager.HasComponent<IndividualSignalLink>(parent)){var link=EntityManager.GetComponentData<IndividualSignalLink>(parent);var pick=World.GetExistingSystemManaged<IndividualSignalData>()?.SignSelection(link.Owner,link.Source,link.Far,kind)??"auto";if(pick!="auto"){candidate=pick=="none"?null:pick;known=true;}}
 ...
 private void Sync(Entry entry){if(Suppressed(entry.Parent)){RetireSigns(entry);return;}int frame=UnityEngine.Time.frameCount;if(entry.SyncFrame==frame)return;entry.SyncFrame=frame;bool live=Live(entry.Parent)&&Live(entry.Owner);bool hidden=!live||EntityManager.HasComponent<Hidden>(entry.Parent)||(EntityManager.HasComponent<KoreanSignalClone>(entry.Parent)&&!SignalRenderReadiness.MainReady(EntityManager,entry.Parent));if(!live){foreach(var e in entry.Signs)Hidden(e,true);return;}
  var t=EntityManager.GetComponentData<Transform>(entry.Parent);for(int i=0;i<entry.Signs.Length;i++){var e=entry.Signs[i];if(!Live(e))continue;if(!hidden)SignalRenderReadiness.Protect(EntityManager,e);var mount=entry.Mounts[i];var next=new Transform{m_Position=t.m_Position+math.rotate(t.m_Rotation,mount.Position),m_Rotation=math.mul(t.m_Rotation,mount.Rotation)};
   if(!next.Equals(EntityManager.GetComponentData<Transform>(e))){EntityManager.SetComponentData(e,next);Mark(e);}Hidden(e,hidden);}
 }
 internal void SyncParent(Entity parent){if(!_saving&&_entries.TryGetValue(parent,out var e)){e.SyncFrame=-1;Sync(e);}}
