...
public partial class ApproachSignalSystem {
 ...
 private bool HasPedestrianDisplay(Entity prefab)=>EntityManager.Exists(prefab)&&EntityManager.HasComponent<TrafficLightData>(prefab)&&((int)EntityManager.GetComponentData<TrafficLightData>(prefab).m_Type&12)!=0;
 ...
 private void ReleasePedestrianMerge(Entry entry){
  ...
  if(entry.OwnsPedestrianHidden&&Live(e)&&PersistentSignalVisibility.HiddenOrOwned(EntityManager,e)){PersistentSignalVisibility.Show(EntityManager,e);Mark(e);}
 ...
 private void RebuildPedestrianMerges(Entity owner){
  ...
  foreach(var pair in local){var source=pair.Key;var entry=pair.Value;if(!Live(source)||!HasPedestrianDisplay(entry.Prefab)||!EntityManager.HasComponent<TrafficLight>(source)||!EntityManager.HasComponent<Transform>(source))continue;
 ...
 private void SyncPedestrianMerge(Entry entry){
  ...
  if(hide&&!EntityManager.HasComponent<Hidden>(e)){PersistentSignalVisibility.Hide(EntityManager,e);entry.OwnsPedestrianHidden=true;Mark(e);}
  else if(!hide&&entry.OwnsPedestrianHidden){if(Live(e)&&PersistentSignalVisibility.HiddenOrOwned(EntityManager,e)){PersistentSignalVisibility.Show(EntityManager,e);Mark(e);}entry.OwnsPedestrianHidden=false;}
 }
 private void SuspendPedestrianMerges(){foreach(var entry in _entries.Values){var e=entry.Pedestrian;if(entry.OwnsPedestrianHidden&&Live(e)&&PersistentSignalVisibility.HiddenOrOwned(EntityManager,e)){PersistentSignalVisibility.Show(EntityManager,e);Mark(e);}entry.OwnsPedestrianHidden=false;}}
}
}
