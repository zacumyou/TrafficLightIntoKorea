...
internal struct BaselineKoreanRenderState:IComponentData {internal bool WasReady;internal float MissingSince;}
internal static class BaselineSignalRenderReadiness {
 internal static void Protect(EntityManager em,Entity e){if(!em.Exists(e)||(!em.HasComponent<KoreanSignalClone>(e)&&!em.HasComponent<KoreanSignVisual>(e))||em.HasComponent<Deleted>(e)||em.HasComponent<KoreanRetired>(e)||em.HasComponent<Temp>(e))return;if(em.HasComponent<Overridden>(e)){em.RemoveComponent<Overridden>(e);if(!em.HasComponent<Updated>(e))em.AddComponent<Updated>(e);if(!em.HasComponent<BatchesUpdated>(e))em.AddComponent<BatchesUpdated>(e);RuntimeDiagnostics.Event("Cleared overlap override on Korean display "+e);}}
 ...
 internal static bool MainReady(EntityManager em,Entity e){if(Allocated(em,e))return true;if(!em.Exists(e)||!em.HasComponent<BaselineKoreanRenderState>(e))return false;var state=em.GetComponentData<BaselineKoreanRenderState>(e);return RenderRecoveryRules.KeepCustom(state.WasReady,Visible(em,e),UnityEngine.Time.realtimeSinceStartup-state.MissingSince);}
 internal static void Repair(EntityManager em,Entity e,ref int attempts,ref float next){
  ...
  if(!em.HasComponent<BaselineKoreanRenderState>(e))em.AddComponentData(e,new BaselineKoreanRenderState{MissingSince=-1});
  var state=em.GetComponentData<BaselineKoreanRenderState>(e);float now=UnityEngine.Time.realtimeSinceStartup;
  if(Allocated(em,e)){if(attempts>0)RuntimeDiagnostics.Event("Custom mesh recovered: clone="+e);state.WasReady=true;state.MissingSince=-1;attempts=0;next=0;em.SetComponentData(e,state);return;}
  if(!Visible(em,e)){state.MissingSince=-1;em.SetComponentData(e,state);return;}
  if(state.MissingSince<0)state.MissingSince=now;em.SetComponentData(e,state);
  if(now<next)return;next=now+RenderRecoveryRules.RetryDelay(attempts);attempts=System.Math.Min(attempts+1,3);
